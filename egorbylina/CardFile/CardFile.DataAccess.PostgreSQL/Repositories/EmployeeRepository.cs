...
namespace CardFile.DataAccess.PostgreSQL.Repositories
{
    ...
    {
        private const string ConnectionString = "host=localhost; port=5433; database=Team; user id=postgres; password=Qwerty123; sslmode=prefer;";
        ...
        private int Insert(Employee employee)
        {
            ...
            using (var connection = new NpgsqlConnection(ConnectionString))
            {
                ...
                using (var cmd = new NpgsqlCommand(query, connection))
                {
                    ...
                    cmd.Parameters.AddWithValue("role", employee.Specialization);
                    cmd.Parameters.AddWithValue("exp", employee.Course);
