...
namespace BinarySearchTree
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var dictionary = new MySortedDictionary<string, int?>();

            for (int i = 0; i < 80; i++)
            {
                var key = rnd.Next(50).ToString();
                var value = rnd.Next(100);
                ...
                Console.WriteLine($"Value (\"{key}\", {value}) was inserted to dictionary.");
            }

            dictionary.ProcessLeftToRight((x, level) => Console.Write($"(\"{x.Key}\", {x.Value}) "));
            Console.WriteLine();
            ...
            for (int i = 0; i < 80; i++)
            {
                ...
                if (value != null)
                {
                    Console.WriteLine($"Key \"{key}\" already exists in dictionary. Its value = {value}.");
                }
                else
                {
